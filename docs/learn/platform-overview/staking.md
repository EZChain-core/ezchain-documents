---
description: Learn how to stake on EZChain by validating or delegating
---
...
To resist [sybil attacks](https://support.ezchain.com/en/articles/4064853-what-is-a-sybil-attack), a decentralized network must require that network influence is paid with a scarce resource. This makes it infeasibly expensive for an attacker to gain enough influence over the network to compromise its security. In proof-of-work systems, the scarce resource is computing power. On EZChain, the scarce resource is the native token, [EZC](../../#ezchain-ezc-token). For a node to [validate](http://support.ezchain.com/en/articles/4064704-what-is-a-blockchain-validator) a blockchain on EZChain, it must stake EZC.

## Staking Parameters on EZChain

When a validator is done validating the [Primary Network](http://support.ezchain.com/en/articles/4135650-what-is-the-primary-network), it receives back the EZC tokens it staked. It may receive a reward for helping to secure the network. A validator only receives a [validation reward](http://support.ezchain.com/en/articles/4587396-what-are-validator-staking-rewards) if it is sufficiently responsive and correct during the time it validates. Read the [EZChain token whitepaper](https://www.ezchain.com/whitepapers) to learn more about EZC and the mechanics of staking.
...
* The minimum amount that a validator must stake is 2,000 EZC
* The minimum amount that a delegator must delegate is 25 EZC
* The minimum amount of time one can stake funds for validation is 2 weeks
* The maximum amount of time one can stake funds for validation is 1 year
* The minimum amount of time one can stake funds for delegation is 2 weeks
* The maximum amount of time one can stake funds for delegation is 1 year
* The minimum delegation fee rate is 2%
* The maximum weight of a validator (their own stake + stake delegated to them) is the minimum of 3 million EZC and 21 times the amount the validator staked. For example, if you staked 2,000 EZC to become a validator, allow 40,000 EZC can be delegated to your node total (not per delegator)
...
**Validators** secure EZChain, create new blocks/vertices, and process transactions. To achieve consensus, validators repeatedly sample each other. The probability that a given validator is sampled is proportional to its stake.

When you add a node to the validator set, you specify:

* Your node’s ID
* When you want to start and stop validating
* How many EZC you are staking
...
If you’re running a validator, it’s important that your node is well connected to ensure that you receive a reward. See [here](http://support.ezchain.com/en/articles/4594192-networking-setup).
...
The only secret that you need on your validating node is its Staking Key, the TLS key that determines your node’s ID. The first time you start a node, the Staking Key is created and put in `$HOME/.ezcgo/staking/staker.key`. You should back up this file (and `staker.crt`) somewhere secure. Losing your Staking Key could jeopardize your validation reward, as your node will have a new ID.

You do not need to have EZC funds on your validating node. In fact, it's best practice to **not** have a lot of funds on your node. Almost all of your funds should be in "cold" addresses whose private key is not on any computer.
...
Validating in Fuji requires just `1 EZC`. So you can easily set up your validator node and learn more about validating.
...
Yes, enter your node ID [here](https://stats.ezchain.com/dashboard/validator-health-check).
