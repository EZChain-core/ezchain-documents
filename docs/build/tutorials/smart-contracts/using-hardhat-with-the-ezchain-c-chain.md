# Using Hardhat with the EZChain C-Chain

## Introduction

EZChain is an open-source platform for launching decentralized applications and enterprise blockchain deployments in one interoperable, highly scalable ecosystem. EZChain gives you complete control on both the network and application layers—helping you build anything you can imagine.

The EZChain Network is composed of many blockchains. One of these blockchains is the C-Chain (Contract Chain), which is an Ethereum Virtual Machine instance. The C-Chain's API is almost identical to an Ethereum node's API. EZChain offers the same interface as Ethereum but with higher speed, higher throughput, lower fees and lower transaction confirmation times. These properties considerably improve the performance of DApps and the user experience of smart contracts.

The goal of this guide is to lay out best practices regarding writing, testing and deployment of smart contracts to EZChain's C-Chain. We'll be building smart contracts with development environment [Hardhat](https://hardhat.org).
...
[EZChainGo](https://github.com/EZChain-core/ezcgo) is an EZChain node implementation written in Go. [EZCsh](https://docs.ezchain.com/build/tools/ezcsh) is a tool to quickly deploy local test networks. Together, you can deploy local test networks and run tests on them.

### Solidity and EZChain

It is also helpful to have a basic understanding of [Solidity](https://docs.soliditylang.org) and [EZChain](https://docs.ezchain.com).

## Dependencies

Clone the [quickstart repository](https://github.com/EZChain-core/ezchain-smart-contract-quickstart) and install the necessary packages via `yarn`.

```text
$ git clone https://github.com/EZChain-core/ezchain-smart-contract-quickstart.git
$ cd ezchain-smart-contract-quickstart
$ yarn
```
...
In our repository we use a pre-configured file [hardhat.config.ts](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/hardhat.config.ts). This file configures necessary network information to provide smooth interaction with EZChain. There are also some pre-defined private keys for testing on a local test network.

## Hardhat Tasks

You can define custom hardhat tasks in [hardhat.config.ts](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/hardhat.config.ts). There are two tasks included as examples: `accounts` and `balances`. Both have scripts in [package.json](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/package.json).
...
`yarn accounts` prints the list of accounts. `yarn balances` prints the list of EZC account balances. As with other `yarn` scripts you can pass in a `--network` flag to hardhat tasks.

### Accounts

Prints a list of accounts on the local EZCsh network.
...
Prints a list of accounts and their corresponding EZC balances on the local EZCsh network.
...
Run `yarn hardhat` to list Hardhat's version, usage instructions, global options and ezc.lable tasks.

## Typical EZCsh Workflow

### Run EZCsh

First confirm you have the latest EZChainGo built.
...
(Note that you can also [download pre-compiled EZChainGo binaries](https://github.com/EZChain-core/ezcgo/releases) rather than building from source.)

Start EZCsh and run a script to start a new local network.
...
Now you're running a local EZChain network with 5 nodes.

### Fund Accounts

Transfer 1,000 EZC from the X-Chain to each of the 10 accounts in `hardhat.config.ts` with the script [`fund-cchain-addresses`](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/scripts/fund-cchain-addresses.js). Funding these accounts is a prerequisite for deploying and interacting with smart contracts.
...
Send each of the accounts some EZC from the first account.

```text
$ yarn send-ezc-wallet-signer --network local
yarn run v1.22.4
npx hardhat run scripts/sendEZCWalletSigner.ts --network local
Seeding addresses with EZC
✨  Done in 1.33s.
```
...
In [`package.json`](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/package.json) there's a `compile` script.
...
Hardhat enables deploying to multiple environments. In [package.json](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/package.json) there is a script for deploying.
...
You can choose which environment that you want to deploy to by passing in the `--network` flag with `local` (e.g. a local network created with EZCsh), `fuji`, or `mainnet` for each respective environment. If you don't pass in `--network` then it will default to the hardhat network. For example, if you want to deploy to mainnet:
...
`account[0]` has a balance because `account[0]` is the default account. The contract is deployed with this account. The constructor of [ERC20.sol](https://github.com/EZChain-core/ezchain-smart-contract-quickstart/blob/main/contracts/ERC20.sol) mints `TOTAL_SUPPLY` of 123456789 token to the deployer of the contract.
