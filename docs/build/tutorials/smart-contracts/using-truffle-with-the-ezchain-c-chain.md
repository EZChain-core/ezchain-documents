# Using Truffle with the EZChain C-Chain

## Introduction

[Truffle Suite](https://www.trufflesuite.com) is a toolkit for launching decentralized applications (dapps) on the EVM. With Truffle you can write and compile smart contracts, build artifacts, run migrations and interact with deployed contracts. This tutorial illustrates how Truffle can be used with EZChain's C-Chain, which is an instance of the EVM.

## Requirements

You've completed [Run an EZChain Node](../nodes-and-staking/run-ezchain-node.md) and are familiar with [EZChain's architecture](../../../learn/platform-overview/README.md). You've also performed a cross-chain swap via the [Transfer EZC Between X-Chain and C-Chain](../platform/transfer-ezc-between-x-chain-and-c-chain.md) tutorial to get funds to your C-Chain address.

## Dependencies

<!-- * [EZChain Network Runner](https://github.com/EZChain-core/ezchain-network-runner) is a tool for running a local EZChain network. It's similar to Truffle's [Ganache](https://www.trufflesuite.com/ganache). -->
* [NodeJS](https://nodejs.org/en) v8.9.4 or later.
* Truffle, which you can install with `npm install -g truffle`

## Start up a local EZChain network

<!-- [EZChain Network Runner](https://github.com/EZChain-core/ezchain-network-runner) allows you to spin up private test network deployments. Start a local five node EZChain network:

```text
cd /path/to/ezchain-network-runner
# start a five node staking network
./go run examples/local/fivenodenetwork/main.go
``` -->

<!-- A five node EZChain network is running on your machine. Network will run until you CTRL + C to exit. -->
...
Use `npm` to install [web3](https://web3js.readthedocs.io), which is a library through which we can talk to the EVM and [EZChainJS](../../tools/ezchainjs/README.md) which is used for cross chain swaps.

```text
npm install web3 ezchain -s
```
...
Note that you can change the `protocol`, `ip` and `port` if you want to direct API calls to a different EZChainGo node. Also note that we're setting the `gasPrice` and `gas` to the appropriate values for the EZChain C-Chain.
...
When deploying smart contracts to the C-Chain, truffle will default to the first ezc.lable account provided by your C-Chain client as the `from` address used during migrations. We have added some pre-defined private keys as our accounts in the `truffle-config.json`. The first and default account should have some pre-funded EZC.
...
There is a convenient script that funds the `accounts` list . You can find it [here](https://raw.githubusercontent.com/EZChain-core/ezchain-documents/master/scripts/fund-cchain-addresses.js). You can also download it using this command:

```text
wget -nd -m https://raw.githubusercontent.com/EZChain-core/ezchain-documents/master/scripts/fund-cchain-addresses.js
```
...
Script will fund 1000 EZC to each account in `accounts` list above. After succesfully running the script you can check balances with:
...
If you wish to fund accounts your own, follow the steps in the [Transfer EZC Between X-Chain and C-Chain](../platform/transfer-ezc-between-x-chain-and-c-chain.md) tutorial. You'll need to send at least `135422040` wei to the account to cover the cost of contract deployments.

### Personal APIs

Personal APIs interact with node’s accounts. `web3` has some functions that uses it, e.g: `web3.eth.personal.newAccount`, `web3.eth.personal.unlockAccount` etc... However this API is disabled by default. It can be activated with `C-chain`/`Coreth` configs. The EZChain Network Runner currently does not support activating this API. So if you want to use these features you need to run your own network manually with `internal-private-personal` API enabled via the `eth-apis` flag. See [Create a Local Test Network/Manually](https://docs.ezchain.com/build/tutorials/platform/create-a-local-test-network#manually) and [C-Chain Configs](https://docs.ezchain.com/build/references/ezcgo-config-flags#c-chain-configs).
...
Now you have the tools you need to launch a local EZChain network, create a truffle project, as well as create, compile, deploy and interact with Solidity contracts.
