...
4. On the next in the dropdown menus select the following
...
// When using the hardhat network, you may choose to fork Fuji or EZChain Mainnet
...
task('balances', 'Prints the list of EZC account balances', async (args, hre): Promise<void> => {
  const accounts: SignerWithAddress[] = await hre.ethers.getSigners()
...
export default {
  etherscan: {
    // Your API key for Explorer if need
    apiKey: APIKEY,
  },
  ...
  networks: {
    hardhat: {
      ...
      chainId: 2612, //Only specify a chainId if we are not forking
      // forking: {
      //   url: 'https://api.ezchain.com/ext/bc/C/rpc',
      // },
    },
