...
EZChain supports creating blockchains with virtual machines in subnets. In this tutorial, we’ll create a custom blockchain using a custom Virtual Machine (Timestamp VM).
...
Every blockchain is an instance of a virtual machine. For example X-Chain is an instance of AVM and C-Chain is EVM's instance. EZChain supports creating new blockchains (instances) from Virtual Machines. In this case we will use [Timestamp VM](https://github.com/EZChain-core/timestampvm), which is an external VM plugin. Timestamp VM will communicate with our EZChainGo node through RPC.
...
You can also alias this chain ID with `timestampbc`, or whatever you like, for simpler API URLs. More information: [admin.aliasChain](https://docs.ezchain.com/build/ezcgo-apis/admin/#adminaliaschain)
