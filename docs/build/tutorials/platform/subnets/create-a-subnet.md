...
**Please checkout [subnet-cli](https://github.com/EZChain-core/subnet-cli#subnet-cli-wizard) for the fast way to create a subnet.** You are welcome to read this document to understand how subnet is created.
...
First, let’s generate the 2 control keys. To do so we call [`platform.createAddress`](../../../ezcgo-apis/p-chain.md#platformcreateaddress) This generates a new private key and stores it for a user.
...
```json
{
    "jsonrpc": "2.0",
    "result": {
        "address": "P-ezc1spnextuw2kfzeucj0haf0e4e08jd4499gn0zwg"
    },
    "id": 1
}
```
...
```json
{
    "jsonrpc": "2.0",
    "result": {
        "address": "P-ezc1zg5uhuwfrf5tv852zazmvm9cqratre588qm24z"
    },
    "id": 1
}
```
...
To create a subnet, we call [`platform.createSubnet`](../../../ezcgo-apis/p-chain.md#platformcreatesubnet).
...
curl -X POST --data '{
    ...
    "params": {
        "controlKeys":[
            "P-ezc1spnextuw2kfzeucj0haf0e4e08jd4499gn0zwg",
            "P-ezc1zg5uhuwfrf5tv852zazmvm9cqratre588qm24z"
        ],
        "threshold":2,
        "username":"USERNAME GOES HERE",
        "password":"PASSWORD GOES HERE"
...
We can call [`platform.getSubnets`](../../../ezcgo-apis/p-chain.md#platformgetsubnets) to get all Subnets that exist:
...
Now let’s add a validator to a subnet. Right now you can only add validators to subnets with API calls, not with EZChain Wallet.
...
To add the validator, we’ll call API method [`platform.addSubnetValidator`](../../../ezcgo-apis/p-chain.md#platformaddsubnetvalidator). Its signature is:
...
The status should be `Committed`, meaning the transaction was successful. We can call [`platform.getPendingValidators`](../../../ezcgo-apis/p-chain.md#platformgetpendingvalidators) and see that the node is now in the pending validator set for the Primary Network. This time, we specify the subnet ID:
...
`./build/ezcgo --whitelisted-subnets=3fbrm3z38NoDB4yMC3hg5pRvc72XqnAGiu7NgaEp1dwZ8AD9g`

For more information about the command see: [whitelisted-subnet command-line argument](../../../references/ezcgo-config-flags.md#whitelist).

### Private Subnets

EZChain subnets are public. It means that every node can sync and listen ongoing transactions/blocks in subnets, even they're not validating the listened subnet.

Subnet validators/beacons can choose not to publish contents of blockchains via an optional `validatorOnly` configuration. The configuration can be turned on with [Subnet Configs](../../../references/ezcgo-config-flags.md#subnet-configs). If a node sets `validatorOnly` to `true`, the node exchanges messages only with this subnet's validators. Other peers will not be able to learn contents of this subnet from this node.
