...
One of the core features of EZChain is the ability to create new blockchains. EZChain supports the creation of new instances of the [EZChain Virtual Machine (AVM)](../../../../learn/platform-overview/README.md#exchange-chain-x-chain). In this tutorial, we’ll create a X-Chain alike blockchain by creating a new instance of the AVM.
...
You will need a running node, a user on the node, and some EZC in the address controlled by the user. All of that is covered in the [Run an EZChain Node](../../nodes-and-staking/run-ezchain-node.md) tutorial.

Next, you need to have your node be a validator on the [Primary Network](http://support.ezchain.com/en/articles/4135650-what-is-the-primary-network). You can find out how to do that in the [Add a Validator](../../nodes-and-staking/add-a-validator.md) tutorial. It is recommended you do that [with API calls](../../nodes-and-staking/add-a-validator.md#add-a-validator-with-api-calls), since that is the way you will be interacting with your node in the rest of this tutorial.
...
The [AVM’s documentation](../../../ezcgo-apis/x-chain.mdx) specifies that the argument to [`avm.buildGenesis`](../../../ezcgo-apis/x-chain.mdx#avm.buildGenesis) should look like this:
...
"genesisData":
    {
        "assetAlias1": {               // Each object defines an asset
            ...
            "symbol":"TESTTOKEN",           // Symbol is between 0 and 4 characters
            "initialState": {
        ...
        "assetAliasCanBeAnythingUnique": { // Asset alias can be used in place of assetID in calls
            ...
            "symbol": "TESTTOKEN",              // symbols need not be unique
            "initialState": {
            ...
            "asset1": {
                ...
                "initialState": {
                    "fixedCap" : [
                        {
                            "amount":100000000,
                            "address": "ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z"
                        },
                        {
                            "amount":100000000,
                            "address": "ezc1u4uvatmymlue3zf4w0admnyj6vsw9mqk7hjckl"
                        },
                        {
                            "amount":5000000,
                            "address": "ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70"
                        },
                        {
                            "amount":5000000,
                            "address": "ezc1hzrwdlpum8xmt3pgstejx4sz86ajylmmaylspc"
                        }
                    ]
                }
            },
            "asset2": {
                ...
                "initialState": {
                    "variableCap" : [
                        {
                            "minters": [
                                "ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70",
                                "ezc1hzrwdlpum8xmt3pgstejx4sz86ajylmmaylspc"
                            ],
                            "threshold":1
                        },
                        {
                            "minters": [
                                "ezc1je76jegcc0qylnz473ag9l5ywvhe8we8e5qw0k",
                                "ezc1y9sull9tpaz9s507uekmm4sejwvndrela0mu43",
                                "ezc1grn5kuzalzek7uk405fmgae06ly8cw52ms070k"
                            ],
                            "threshold":2
                        }
                    ]
...
Now let’s create the new blockchain. To do so, we call [`platform.createBlockchain`](../../../ezcgo-apis/p-chain.md#platformcreateblockchain). Your call should look like the one below. You have to change `subnetID` to the subnet that will validate your blockchain, and supply a `username` that controls a sufficient number of the subnet’s control keys. As a reminder, you can find out what a subnet’s threshold and control keys are by calling [`platform.getSubnets`](../../../ezcgo-apis/p-chain.md#platformgetsubnets).
...
To check, call [`platform.getBlockchains`](../../../ezcgo-apis/p-chain.md#platformgetblockchains). This returns a list of all blockchains that exist.
...
Every blockchain needs a set of validators to validate and process transactions on it. You can check if a node is validating a given blockchain by calling [`platform.getBlockchainStatus`](../../../ezcgo-apis/p-chain.md#platformgetblockchainstatus) on that node:
...
* The API endpoint of your blockchain is `127.0.0.1:9650/ext/bc/xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH`. The last part in the endpoint is the blockchain ID. This can be a different ID when you create your blockchain. You can also alias this chain ID with `myxchain` for simpler API URLs. More information:
  ...
  [admin.aliasChain](https://docs.ezchain.com/build/ezcgo-apis/admin#admin-aliaschain)
...
* Fees are paid with the first asset specified in the genesis data, as noted above, rather than EZC...

### Verify Balance

In the genesis data we specified that address `ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z` has 100,000,000 units of the asset with alias `asset1`. Let’s verify that:
...
curl -X POST --data '{
    ...
    "params" :{
        "address":"xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z",
        "assetID":"asset1"
    }
...
```json
{
  "jsonrpc": "2.0",
  "result": {
    "address": "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc1u4uvatmymlue3zf4w0admnyj6vsw9mqk7hjckl"
  },
  "id": 1
}
```

Now let's send 1 unit of `asset1` to the new address with [`avm.send`](../../../ezcgo-apis/x-chain.mdx#avm.send).
...
curl -X POST --data '{
    ...
    "params" :{
        "assetID" : "asset1",
        "amount"  : 1,
        "from"    : ["xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z"],
        "to"      : "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc1u4uvatmymlue3zf4w0admnyj6vsw9mqk7hjckl",
        "changeAddr": "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z",
        "username": "USERNAME GOES HERE",
        "password": "PASSWORD GOES HERE"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/myxchain
...
```json
{
  ...
  "result": {
    ...
    "changeAddr": "g1GK7GErN3BqauK6BhhU8uCNfaBTMz4VWr3JdwvXXNCwpwQJQ-ezc1dmrwka6uck44zkaamagq46hhntta67yxfy9h9z"
  },
  "id": 1
}
```
...
Our blockchain has another asset `asset2` named `myVarCapAsset`. It is a variable-cap asset. Let's mint more units of this asset with [`avm.mint`](../../../ezcgo-apis/x-chain.mdx#avm.mint). Address `ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70` controls the mintable asset `asset2`, and it also has 5,000,000 unit `asset1`, which is enough to pay the transaction fee.
...
curl -X POST --data '{
    ...
    "params" :{
        "amount": 1,
        "assetID": "asset2",
        "from": ["xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70"],
        "to": "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70",
        "minters": [
            "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70"
        ],
        "changeAddr": "xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70",
        "username": "USERNAME GOES HERE",
        "password": "PASSWORD GOES HERE"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/myxchain
...
Let's check the balance with [`avm.getAllBalances`](../../../ezcgo-apis/x-chain.mdx#avm.getAllBalances).
...
curl -X POST --data '{
    ...
    "params" :{
        "address":"xAd5n5PQFV6RRo8UgH54Gf5tJs8oQdctQS2ygp5F2dKZDckYH-ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/myxchain
```
...
As we can see, 1 unit of `asset2` was minted. Address `ezc16k8n4d8xmhplqn5vhhm342g6n9rkxuj8wn6u70` had 5,000,000 `asset1`, as defined in the genesis data, and now has 4,000,000 `asset1` after paying the transaction fee.
