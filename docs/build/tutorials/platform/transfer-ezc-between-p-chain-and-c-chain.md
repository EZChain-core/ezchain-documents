...
You will be presented with a choice for **Source Chain** and **Destination Chain**. Select P-Chain and C-Chain, respectively. You will see your source chain's balance, and an input field for entering the amount to transfer from source to destination chain. If you do not have any balance on your P-Chain but on X-Chain, then you can follow this [tutorial](./transfer-ezc-between-x-chain-and-p-chain.md) to transfer it to P-Chain.
...
That's it! You've transferred EZC from the P-Chain to C-Chain! Now you can use them to deploy smart contracts on C-Chain or pay for transaction fees while interacting with smart contracts.

### Transfer from C-Chain to P-Chain

To return the EZC back to the P-Chain, you need to do the transfer in the opposite direction.
...
If you're building an application on the EZChain network, you may want to do the transfer programmatically as part of some broader functionality. You can do that by calling the appropriate APIs on an EZChainGo node. The rest of the tutorial assumes you have access to an EZChainGo node, EZC tokens on the P-Chain, and user credentials [created](../../ezcgo-apis/keystore.md#keystorecreateuser) and stored in the node's keystore.
...
* Export EZC from the P-Chain (source)
* Import EZC to the C-chain (destination)
...
curl -X POST --data '{
    ...
    "params" :{
        "username" :"myUsername",
        "password":"myPassword",
        "address": "X-ezc1fw57u4tp7xzx0k6ufn7tj9caua59mt9gqcvy7m"
    }
...
Use the address corresponding to the private key you exported and switch to using the C- prefix in the [`platform.exportEZC`](../../ezcgo-apis/p-chain.md#platformexportezc) call:
...
curl -X POST --data '{  
    ...
    "params" :{ 
        "assetID": "EZC",
        "to": "C-ezc1fw57u4tp7xzx0k6ufn7tj9caua59mt9gqcvy7m"
        "amount": 5000000,  
        "username":"myUsername",    
        "password":"myPassword" 
    }   
...
Once your EZC has been transferred to the C-Chain, you can use it to deploy and interact with smart contracts.

## Transfer from the C-Chain to P-Chain

Now, you can move EZC back from the C-Chain to the P-Chain. First we need to export:
...
curl -X POST --data '{  
    ...
    "params" :{ 
        "to":"P-ezc1fw57u4tp7xzx0k6ufn7tj9caua59mt9gqcvy7m",   
        "assetID": "EZC",
        "amount": 5000000,  
        "username":"myUsername",    
        "password":"myPassword" 
...
To finish the transfer, call P-Chain’s [`platform.importEZC`](../../ezcgo-apis/p-chain.md#platformimportezc)
...
That’s it! Now, you can swap EZC back and forth between the P-Chain and C-Chain, both by using the EZChain Wallet, and by calling the appropriate API calls on an EZChain node.
