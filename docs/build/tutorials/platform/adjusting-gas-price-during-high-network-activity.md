...
The amount of computation used by a transaction is measured in units of `gas`. Each unit of gas is paid for in EZC at the `gas price` for the transaction. The `gas price` of the transaction is determined by the parameters of the transaction and the `base fee` of the block that it is included in.
...
Consider the following examples (here GWEI or wei is one-billionth of EZC) -
...
1. Let's create a sample transaction on EZChain Mainnet, in which we will be sending 0.1 EZC to a receiver using the MetaMask. By clicking **Next** we can review gas fees and the amount which we want to send.
