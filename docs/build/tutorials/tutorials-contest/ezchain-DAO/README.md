# How to Create Your Own DAO with EZChain
...
> We need to setup the FUJI Testnet on our Metamask. [`find here the rpc values`](https://docs.ezchain.com/build/tutorials/smart-contracts/deploy-a-smart-contract-on-ezchain-using-remix-and-metamask)
...
![MyDAO.sol](./assets/ezchain-dao-newFile-remix.png)
...
![](assets/ezchain-dao-firstLines.png)
...
We use EZC our governance token.
...
// the IERC20 allow us to use ezc like our governance token.
IERC20 public token;
// the user need minimum 25 EZC to create a proposal.
...
We already have our necessary variables to create, save and vote a proposal in our DAO, now we need our user deposit his `EZC` tokens to avoid that the same user can use the same amount of tokens for vote other option in the same proposal.
To interact with EZC as our token the governance we need to initialize the token address in the constructor.

```solidity
constructor() {
    token = IERC20(0xA048B6a5c1be4b81d99C3Fd993c98783adC2eF70); // EZC address
}
```
...
For our `createProposal` function we will add the condition that if the user does not have minimum 25 EZC tokens He cannot create a new proposal.
...
![](assets/ezchain-dao-remix-compiler.png)

In the environment section we choose the `Injected Web3` option, in account we chose an account from our metamask plugin in the FUJI network, make sure that your account have the necessary ezc for the deploy and the minimum for create a proposal.   
[Here you can find the Faucet](https://testnet-faucet.ezchain.com/).  
Click on the `Deploy` button and confirm the transaction in REMIX and Metamask and await for a few seconds.  

![](assets/ezchain-dao-deploy-smartContract.png)

![](assets/ezchain-dao-confirm-deploy.png)

![](assets/ezchain-dao-metamask.png)

If the contract is deployed successfully on FUJI we can see the succes transaction on the REMIX inspector.

![](assets/ezchain-dao-remix-Tx.png)
