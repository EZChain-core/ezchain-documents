...
Instead of having to back up and restore everything, we need to back up only what is essential, that is, those files that cannot be reconstructed because they are unique to your node. For EZChaingo node, unique files are those that identify your node on the network, in other words, files that define your NodeID. 
...
:::caution
If you have users defined in the keystore of your node, then you need to back up and restore those as well. [Keystore API](../../ezcgo-apis/keystore.md) has methods that can be used to export and import user keys. Note that Keystore API is used by developers only and not intended for use in production nodes. If you don't know what a keystore API is and have not used it, you don't need to worry about it.
:::
...
On a default Linux installation, the path to them will be `/home/USERNAME/.ezcgo/staking/`, where `USERNAME` needs to be replaced with the actual username running the node. Select and copy the files from there to a backup location. You don't need to stop the node to do that.

#### From Remote Node Using `scp`

`scp` is a 'secure copy' command line program, ezc.lable built-in on Linux and MacOS computers. There is also a Windows version, `pscp`, as part of the [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) package. If using `pscp`, in the following commands replace each usage of `scp` with `pscp -scp`.

To copy the files from the node, you will need to be able to remotely log into the machine. You can use account password, but the secure and recommended way is to use the SSH keys. The procedure for acquiring and setting up SSH keys is highly dependent on your cloud provider and machine configuration. You can refer to our [Amazon Web Services](setting-up-an-ezchain-node-with-amazon-web-services-aws.md) and [Microsoft Azure](set-up-an-ezchain-node-with-microsoft-azure.md) setup guides for those providers. Other providers will have similar procedures.

When you have means of remote login into the machine, you can copy the files over with the following command:

```text
scp -r ubuntu@PUBLICIP:/home/ubuntu/.ezcgo/staking ~/ezchain_backup
```
...
```text
scp -i /path/to/the/key.pem -r ubuntu@PUBLICIP:/home/ubuntu/.ezcgo/staking ~/ezchain_backup
```

Once executed, this command will create `ezchain_backup` directory in you home directory and place staker files in it. You need to store them somewhere safe.
...
If you're running the node locally, just copy the `staker.key` and `staker.crt` files from the backup location into the working directory, which on the default Linux installation will be `/home/USERNAME/.ezcgo/staking/`. Replace `USERNAME` with the actual username used to run the node.
...
```text
scp ~/ezchain_backup/staker.* ubuntu@PUBLICIP:/home/ubuntu/.ezcgo/staking
```

Or if you need to specify the path to the SSH key:

```text
scp -i /path/to/the/key.pem ~/ezchain_backup/staker.* ubuntu@PUBLICIP:/home/ubuntu/.ezcgo/staking
```
...
You can now check that the node is restored with the correct NodeID by issuing the [getNodeID](https://docs.ezchain.com/build/ezcgo-apis/info#info-getnodeid) API call in the same console you ran the previous command:
...
First, make sure to stop EZChainGo, run:
...
:::warning
You must stop the EZChain node before you back up the database otherwise data
...
```
zip -r ezchain_db_backup.zip .ezcgo/db
```
_Note: It may take > 30 minutes to zip the node's DB._

Next, you can transfer the backup to another machine:
```
scp -r ubuntu@PUBLICIP:/home/ubuntu/ezchain_db_backup.zip ~/ezchain_db_backup.zip
```
This assumes the username on the machine is `ubuntu`, replace with correct username in both places if it is different. Also, replace `PUBLICIP` with the actual public IP of the machine. If `scp` doesn't automatically use your downloaded SSH key, you can point to it manually:

```
scp -i /path/to/the/key.pem -r ubuntu@PUBLICIP:/home/ubuntu/ezchain_db_backup.zip ~/ezchain_db_backup.zip
```
Once executed, this command will create `ezchain_db_backup.zip` directory in you home directory.


### Database Restore

_This tutorial assumes you have already completed "Database Backup" and have
a backup at ~/ezchain_db_backup.zip._
...
:::warning
You must stop the EZChain node before you restore the database otherwise data
could become corrupted.
:::
...
```
mv .ezcgo/db .ezcgo/db-old
```

Next, we'll unzip the backup we moved from another node (this will place the
unzipped files in `~/.ezcgo/db` when the command is run in the home directory):

```
unzip ezchain_db_backup.zip
```
...
```
ssh -i /path/to/the/key.pem ubuntu@PUBLICIP 'tar czf - .ezcgo/db' | tar xvzf - -C .
```
...
After copying is done, all you need to do now is move the database to the correct location on the destination machine. Assuming there is a default EZChainGo node installation, we remove the old database and replace it with the new one:
...
If you have any problems following this tutorial, comments you want to share with us or just want to chat, you can reach us on our [Telegram](https://t.me/EZChainOfficia/) server.
