...
```text
cd
cp ~/.ezcgo/staking/staker.crt .
cp ~/.ezcgo/staking/staker.key .
```
...
`sudo systemctl stop ezcgo.service`

(your service may be named differently, `ezchain.service`, or similar)
...
Building the node [from source](upgrade-your-ezcgo-node.mdx#build-from-source) is recommended if you're a developer looking to experiment and build on EZChain.
...
For MacOS:  
Download: `ezcgo-macos-<VERSION>.zip`  
Unzip: `unzip ezcgo-macos-<VERSION>.zip`  
The resulting folder, `ezcgo-<VERSION>`, contains the binaries.

For Linux on PCs or cloud providers:  
Download: `ezcgo-linux-amd64-<VERSION>.tar.gz`  
...
Download: `ezcgo-linux-arm64-<VERSION>.tar.gz`  
...
`cp -r ezcgo-<VERSION>-linux/* <DIRECTORY_WITH_OLD_BINARIES>`

and then restart the service with `sudo systemctl start ezcgo.service`.
...
Then move to the ezcgo directory:
...
and check that the first 7 characters printed match the Latest commit field on our [Github.](https://github.com/EZChain-core/ezcgo)
