...
The [Primary Network](https://support.ezchain.com/en/articles/4135650-what-is-the-primary-network) is inherent to the EZChain platform and validates EZChain’s [built-in blockchains](../../../learn/platform-overview/README.md). In this tutorial, we’ll add a node to the Primary Network on EZChain.

The P-Chain manages metadata on EZChain. This includes tracking which nodes are in which subnets, which blockchains exist, and which subnets are validating which blockchains. To add a validator, we’ll issue [transactions](http://support.ezchain.com/en/articles/4587384-what-is-a-transaction) to the P-Chain.

:::danger
Note that once you issue the transaction to add a node as a validator, there is no way to change the parameters. **You can’t remove your stake early or change the stake amount, node ID, or reward address.** Please make sure you’re using the correct values in the API calls below. If you’re not sure, browse the [Developer FAQ's](http://support.ezchain.com/en/collections/2618154-developer-faq) or ask for help on [Telegram.](https://t.me/EZChainOfficia/)
:::

## Requirements

You've completed [Run an EZChain Node](run-ezchain-node.md) and are familiar with [EZChain's architecture](../../../learn/platform-overview/README.md). In this tutorial, we use [EZChain’s Postman collection](https://github.com/EZChain-core/ezchain-postman-collection) to help us make API calls.
...
First, we show you how to add your node as a validator by using [EZChain Wallet](https://wallet.ezchain.com).

Get your node’s ID by calling [`info.getNodeID`](../../ezcgo-apis/info.md#infogetnodeid):
...
Calling [`platform.getPendingValidators`](../../ezcgo-apis/p-chain.md#platformgetpendingvalidators) verifies that our transaction was accepted.
...
We can also add a node to the validator set by making API calls to our node. To add a node the Primary Network, we’ll call [`platform.addValidator`](../../ezcgo-apis/p-chain.md#platformaddvalidator).
...
This is the node ID of the validator being added. To get your node’s ID, call [`info.getNodeID`](../../ezcgo-apis/info.md#infogetnodeid):
...
In order to validate the Primary Network, one must stake EZC. This parameter defines the amount of EZC staked.
...
EZChain allows for delegation of stake. This parameter is the percent fee this validator charges when others delegate stake to them. For example, if `delegationFeeRate` is `1.2345` and someone delegates to this validator, then when the delegation period is over, 1.2345% of the reward goes to the validator and the rest goes to the delegator.

`username` and `password`

These parameters are the username and password of the user that pays the transaction fee, provides the staked EZC, and to whom the staked EZC will be returned.

Now let’s issue the transaction. We use the shell command `date` to compute the Unix time 10 minutes and 30 days in the future to use as the values of `startTime` and `endTime`, respectively. (Note: If you’re on a Mac, replace `$(date` with `$(gdate`. If you don’t have `gdate` installed, do `brew install coreutils`.) In this example we stake 2,000 EZC (2 x 1012 wei).
...
curl -X POST --data '{
    ...
    "params": {
        ...
        "rewardAddress":"P-ezc1d4wfwrfgu4dkkyq7dlhx0lt69y2hjkjeejnhca",
...
We can check the transaction’s status by calling [`platform.getTxStatus`](../../ezcgo-apis/p-chain.md#platformgettxstatus):
...
The status should be `Committed`, meaning the transaction was successful. We can call [`platform.getPendingValidators`](../../ezcgo-apis/p-chain.md#platformgetpendingvalidators) and see that the node is now in the pending validator set for the Primary Network:
...
When the time reaches `1584021450`, this node will start validating the Primary Network. When it reaches `1584121156`, this node will stop validating the Primary Network. The staked EZC will be returned to an address controlled by `username`, and the rewards, if any, will be given to `rewardAddress`.
