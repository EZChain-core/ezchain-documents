---
description: EZChain also supports non-fungible tokens (NFTs). By definition, each NFT is unique and not perfectly interchangeable for any other NFT.
---
...
On EZChain, digital goods are represented as tokens. Some tokens are **fungible**, which means that one token is interchangeable for any other one token. Real-world currency is fungible, for example; one $5 note is treated as being the same as any other $5 note.

EZChain also supports non-fungible tokens (NFTs). By definition, each NFT is unique and not perfectly interchangeable for any other NFT. For example, there could be an NFT that represents ownership of a real-world piece of art; each piece of art, like each NFT, is unique. NFTs represent digital scarcity and may prove to have even greater utility than traditional fungible tokens.

In this tutorial, we’ll create and send NFTs using EZChainGo’s API. In a future tutorial, we’ll create a custom NFT family using [EZChainJS](../../tools/ezchainjs/README.md) and explore NFTs in more detail.

## Requirements

You've completed [Run an EZChain Node](../nodes-and-staking/run-ezchain-node.md) and are familiar with [EZChain's architecture](../../../learn/platform-overview/README.md). In this tutorial, we use [EZChain’s Postman collection](https://github.com/EZChain-core/ezchain-postman-collection) to help us make API calls.

## Create the NFT Family

Each NFT belongs to a **family**, which has a name and a symbol. Each family is composed of **groups**. The number of groups in a family is specified when the family is created. Our NFT will exist on the X-Chain, so to create our NFT family we’ll call [`avm.createNFTAsset`](../../ezcgo-apis/x-chain.mdx#avmcreatenftasset), which is a method of the [X-Chain’s API](../../ezcgo-apis/x-chain.mdx).
...
* [`avm.createNFTAsset`](../../ezcgo-apis/x-chain.mdx#avmcreatenftasset)
...
curl -X POST --data '{
    ...
    "params" :{
        ...
        "minterSets":[
            {
                "minters": [
                    "X-ezc1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7"
                ],
                "threshold": 1
            }
        ],
...
NFT outputs don’t show up in calls to [`avm.getBalance`](../../ezcgo-apis/x-chain.mdx#avmgetbalance) or [`avm.getAllBalances`](../../ezcgo-apis/x-chain.mdx#avmgetallbalances). To see your NFTs, you have to call [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) and then parse the utxo to check for the type ID. NFT Mint Outputs have a type id of `00 00 00 0a` in hexidecimal (`10` in decimal) and NFT Transfer Outputs have a type id of `00 00 00 0b` in hexdecimal (`11` in decimal).

### **Method**

* [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos)
...
curl -X POST --data '{
    ...
    "method" :"avm.getUTXOs",
    "params" :{
        "addresses":["X-ezc1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7"]
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
...
```json
{
    ...
    "result" :{
        ...
        "endIndex": {
            "address": "X-ezc1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7",
...
[`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) returns 2 UTXOs. Let’s take the first one and decode it to confirm that it’s an [NFT Mint Output.](../../references/avm-transaction-serialization.md#nft-mint-output) First, we convert the Base58Check encoded string which is returned from [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) in to hex. The following [CB58](http://support.ezchain.com/en/articles/4587395-what-is-cb58) string:
...
Now that we have an NFT family and a group for the single `MinterSet` we’re able to create NFTs belonging to this group. To do that we call [`avm.mintNFT`](../../ezcgo-apis/x-chain.mdx#avmmintnft):

### **Method**

* [`avm.mintNFT`](../../ezcgo-apis/x-chain.mdx#avmmintnft)

### **Parameters**

* `assetID` is the ID of the NFT family.
* `payload` is an arbitrary CB58 encoded payload of up to 1024 bytes. In Part 2 (**COMING SOON**) we’ll explore creating a protocol around the NFT payload. For this tutorial, the payload is the string "EZC Labs".
...
curl -X POST --data '{
    ...
    "params" :{
        ...
        "to":"X-ezc1a202a8pu5w4vnerwzp84j68yknm6lf47drfsdv",
        "username":"USERNAME GOES HERE",
        "password":"PASSWORD GOES HERE"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
...
```json
{
    ...
    "result" :{
        ...
        "changeAddr": "X-ezc1a202a8pu5w4vnerwzp84j68yknm6lf47drfsdv"
    }
}
```

Similar to the previous step, we can now confirm that an NFT was minted by calling [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) and parsing the UTXO to confirm that we now have an [NFT Transfer Output](../../references/avm-transaction-serialization.md#nft-transfer-output).
...
curl -X POST --data '{
    ...
    "method" :"avm.getUTXOs",
    "params" :{
        "addresses":["X-ezc1a202a8pu5w4vnerwzp84j68yknm6lf47drfsdv"]
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
...
```json
{
    ...
    "result": {
        "numFetched": "2",
        "utxos": [
            "11Do4RK6FchGXeoycKujR7atm3tvBz3qc64uoipCc5J74Sj1U4orM6vbBGSES8hnjgjZezc.oPgmnbHxh2mBKjeXdvAqTRtYMHEacrveSzKgk7F8h8xi8JB9CddoiX8nbjZMYt1keGo5Rvpjh8dGymDWwRbV1FdnG5uDiiyU8uidc3P24",
        ...
        "endIndex": {
            "address": "X-ezc1a202a8pu5w4vnerwzp84j68yknm6lf47drfsdv",
...
First, we convert the Base58Check encoded string which is returned from [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) in to hex. The following CB58 string:
...
Now, you can send the NFT to anyone. To do that, use EZChainGo’s [`avm.sendNFT`](../../ezcgo-apis/x-chain.mdx#avmsendnft) API method.

**Method**

* [`avm.sendNFT`](../../ezcgo-apis/x-chain.mdx#avmsendnft)
...
curl -X POST --data '{
    ...
    "params" :{
        "assetID" :"2X1YV4jpGpqezvj2khQdj1yEiXU1dCwsJ7DmNhQRyZZ7j9oYBp",
        "to"      :"X-ezc1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7",
        "groupID" : 0,
        "username":"USERNAME GOES HERE",
        "password":"PASSWORD GOES HERE"
    }
...
```json
{
    ...
    "result" :{
        ...
        "changeAddr": "X-ezc1a202a8pu5w4vnerwzp84j68yknm6lf47drfsdv"0
    }
}
```

You can call [`avm.getUTXOs`](../../ezcgo-apis/x-chain.mdx#avmgetutxos) for the address which you sent the NFT to and decompose the returned UTXO, after converting from CB58 to hex, to confirm that there is a UTXO with type id `00 00 00 0b` in hex or `11` in decimal.
...
In Part 2 of this series, we’ll go more in-depth by using EZChainJS to create a protocol for our NFT payload by issuing it to multiple groups.
