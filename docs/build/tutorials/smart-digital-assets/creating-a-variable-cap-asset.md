...
Our asset will exist on the X-Chain, so to create our asset we’ll call [`avm.createVariableCapAsset`](../../ezcgo-apis/x-chain.mdx#avmcreatevariablecapasset), which is a method of the [X-Chain’s API](../../ezcgo-apis/x-chain.mdx).
...
curl -X POST --data '{
    ...
    "params" :{
        ...
        "minterSets":[
            ...
            {
                "minters": [
                    "X-ezc1k4nr26c80jaquzm9369j5a4shmwcjn0vmemcjz",
                    "X-ezc1yell3e4nln0m39cfpdhgqprsd87jkh4qnakklx",
                    "X-ezc1ztkzsrjnkn0cek5ryvhqswdtcg23nhge3nnr5e"
                ],
                "threshold": 2
            }
        ],
...
We’ll use [`avm.mint`](../../ezcgo-apis/x-chain.mdx#avmmint) to mint the shares.
...
We can check the status of the transaction we’ve just sent to the network using [`avm.getTxStatus`](../../ezcgo-apis/x-chain.mdx#avmgettxstatus):
...
All 10M shares are controlled by the `to` address we specified in `mint`. To verify this, we’ll use [`avm.getBalance`](../../ezcgo-apis/x-chain.mdx#avmgetbalance):
...
Let’s send 100 shares to another address by using [`avm.send`](../../ezcgo-apis/x-chain.mdx#avmsend). To do so:
...
curl -X POST --data '{
    ...
    "params" :{
        ...
        "assetID" :"i1EqsthjiFTxunrj8WD2xFSrQ5p2siEKQacmCCB5qBFVqfSL2",
        "amount"  :100,
        "to"      :"X-ezc1qwnlpknmdkkl22rhmad0dcn80wfasp2y3yg3x0"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
...
curl -X POST --data '{
    ...
    "method" :"avm.getBalance",
    "params" :{
        "address":"X-ezc1qwnlpknmdkkl22rhmad0dcn80wfasp2y3yg3x0",
        "assetID":"i1EqsthjiFTxunrj8WD2xFSrQ5p2siEKQacmCCB5qBFVqfSL2"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
