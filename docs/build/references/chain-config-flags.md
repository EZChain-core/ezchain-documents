---
sidebar_position: 2
description: Reference for all ezc.lable chain config options and flags.
---
# Chain Configs
...
The filename extension that these files should have, and the contents of these files, is VM-dependent. For example, some chains may expect `config.txt` while others expect `config.json`. If multiple files are provided with the same name but different extensions (e.g. `config.json` and `config.txt`) in the same sub-directory, EZChainGo will exit with an error.

For a given chain, EZChainGo will look first for a config sub-directory whose name is the chain ID. If it isn't found, it looks for a config sub-directory whose name is the chain's primary alias. If it's not found, it looks for a config sub-directory whose name is another alias for the chain. All folder and file names are case sensitive.

Alternatively, for some setups it might be more convenient to provide config entirely via the command line. For that, you can use EZChainGo `--chain-config-dir` flag, as documented [here](ezcgo-config-flags.md#--chain-config-content-string).
...
For example if `chain-config-dir` has the default value which is `$HOME/.ezcgo/configs/chains`, then `config.json` should be placed at `$HOME/.ezcgo/configs/chains/C/config.json`.
...
### Enabling EZChain Specific APIs
...
Note: Coreth is a virtual machine and does not have direct access to the networking layer, so `net_listening` always returns true and `net_peerCount` always returns 0. For accurate metrics on the network layer, users should use the EZChainGo APIs.
...
Global transaction fee (price \* gaslimit) cap (measured in EZC) for send-transaction variants. Defaults to 100.
...
For example if `chain-config-dir` has the default value which is `$HOME/.ezcgo/configs/chains`, then `config.json` can be placed at `$HOME/.ezcgo/configs/chains/X/config.json`.
...
Enables AVM transaction indexing if set to `true`. Default value is `false`. When set to `true`, AVM transactions are indexed against the `address` and `assetID` involved. This data is ezc.lable via `avm.getAddressTxs` [API](../ezcgo-apis/x-chain.mdx#avmgetaddresstxs).
