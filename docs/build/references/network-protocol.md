...
EZChain network defines the core communication format between EZChain nodes. It uses the [primitive serialization](serialization-primitives.md) format for payload packing.
...
`Version` ensures that the nodes we are connected to are running compatible versions of EZChain, and at least loosely agree on the current time.
...
Sending a `Version` message with the time `November 16th, 2008 at 12:00am (UTC)` and the version `ezchain/0.0.1`

```text
[
    Long   <- 1226793600 = 0x00000000491f6280
    String <- "ezchain/0.0.1"
]
=
[
