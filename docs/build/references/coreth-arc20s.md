...
sidebar_position: 3
---
# EZChain Native Tokens and ARC-20s

## What is an EZChain Native Token?

An EZChain Native Token (ANT) is a fixed-cap or variable-cap token created on the X-Chain. These tokens can be exchanged at lightning fast speeds on the X-Chain, which takes advantage of the superior performance of a DAG over a linear chain. In this document, EZChain Native Tokens do not include non-fungible tokens (NFTs) created on the X-Chain.
...
EZC plays the same role on the C-Chain that ETH does on the Ethereum Network. When you create or call a smart contract, you pay the transaction fee (gas cost) with EZC. You can transfer EZC between accounts and send EZC to a smart contract using native EVM tools and libraries.
...
* **`gasPrice`** Scalar value equal to the number of Wei (1 Wei = 10^-18 EZC) paid per unit of gas to execute this transaction.
...
* **`value`** Scalar value of native asset (EZC), in Wei (1 Wei = 10^-18 EZC), to be transferred to the message call's recipient or in the case of a contract creation, as an endowment to the newly created contract.
...
```text
curl --location --request POST 'https://api.ezchain.com:443/ext/bc/C/rpc' \
...
`nativeAssetBalance` is a precompiled contract at address `0x0100000000000000000000000000000000000001`. `nativeAssetBalance` is the ANT equivalent of using `balance` to get the EZC balance.
...
An ARC-20 is an ERC-20 token that wraps an underlying EZChain Native Token, similar to how WEZC wraps EZC.
...
Unlike ERC-20s, EZChain Native Tokens (ANTs) are stored directly on the account that owns them. ANTs can be "wrapped" in order to make them usable in smart contracts on the C-Chain. We call this wrapped asset an ARC-20. To do this, we add an `assetID` field to a regular ERC-20 contract to represent the underlying asset that the ARC-20 wraps.
