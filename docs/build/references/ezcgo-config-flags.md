---
sidebar_position: 1
description: This documents list all ezc.lable configuration and flags for EZChainGo.
---

# EZChainGo Config and Flags
...
If set to `false`, this node will not expose the Admin API. Defaults to `false`. See [here](../ezcgo-apis/admin.md) for more information.

#### `--api-auth-required` (boolean):

If set to `true`, API calls require an authorization token. Defaults to `false`. See [here](../ezcgo-apis/auth.md) for more information.

#### `--api-auth-password` (string):

The password needed to create/revoke authorization tokens. If `--api-auth-required=true`, must be specified; otherwise ignored. See [here](../ezcgo-apis/auth.md) for more information.

#### `--api-health-enabled` (boolean):

If set to `true`, this node will expose the Health API. Defaults to `true`. See [here](../ezcgo-apis/health.md) for more information.

#### `--index-enabled` (boolean): {#index-enabled}

If `false`, this node will not enable the indexer and the Index API will not be ezc.lable. Defaults to `false`. See [here](../ezcgo-apis/index-api.md) for more information.

#### `--api-info-enabled` (boolean):

If set to `true`, this node will expose the Info API. Defaults to `true`. See [here](../ezcgo-apis/info.md) for more information.

#### `--api-ipcs-enabled` (boolean):

If set to `true`, this node will expose the IPCs API. Defaults to `false`. See [here](../ezcgo-apis/ipc.md) for more information.

#### `--api-keystore-enabled` (boolean):

If set to `false`, this node will not expose the Keystore API. Defaults to `true`. See [here](../ezcgo-apis/keystore.md) for more information.

#### `--api-metrics-enabled` (boolean):

If set to `false`, this node will not expose the Metrics API. Defaults to `true`. See [here](../ezcgo-apis/metrics.md) for more information.
...
Some chains allow the node operator to provide a custom configuration. EZChainGo can read chain configurations from files and pass them to the corresponding chains on initialization.
...
Specifies the directory that contains chain configs, as described [here](chain-config-flags.md). Defaults to `$HOME/.ezcgo/configs/chains`. If this flag is not provided and the default directory does not exist, EZChainGo will not exit since custom configs are optional. However, if the flag is set, the specified folder must exist, or EZChainGo will exit with an error. This flag is ignored if `--chain-config-content` is specified.
...
Specifies the directory to which the database is persisted. Defaults to `"$HOME/.ezcgo/db"`.
...
Each node runs an HTTP server that provides the APIs for interacting with the node and the EZChain network. This argument specifies the port that the HTTP server will listen on. The default value is `9650`.
...
Specifies the directory in which system logs are kept. Defaults to `"$HOME/.ezcgo/logs"`.
...
EZChain uses Proof of Stake (PoS) as Sybil resistance to make it prohibitively expensive to attack the network. If false, sybil resistance is disabled and all peers will be sampled during consensus. Defaults to `true`. Note that this can not be specified on public networks (`Fuji` and `Mainnet`).
...
EZChain uses two-way authenticated TLS connections to securely connect nodes. This argument specifies the location of the TLS certificate used by the node. By default, the node expects the TLS certificate to be at `$HOME/.ezcgo/staking/staker.crt`. This flag is ignored if `--staking-tls-cert-file-content` is specified.
...
EZChain uses two-way authenticated TLS connections to securely connect nodes. This argument specifies the location of the TLS private key used by the node. By default, the node expects the TLS private key to be at `$HOME/.ezcgo/staking/staker.key`. This flag is ignored if `--staking-tls-key-file-content` is specified.
...
It is possible to provide parameters for subnets. Parameters here apply to all chains in the specified subnets. Parameters must be specified with a `{subnetID}.json` config file under `--subnet-config-dir`. EZChainGo loads configs for subnets specified in `--whitelisted-subnet` parameter.

#### `--subnet-config-dir` (string):

Specifies the directory that contains subnet configs, as described above. Defaults to `$HOME/.ezcgo/configs/subnets`. If the flag is set explicitly, the specified folder must exist, or EZChainGo will exit with an error. This flag is ignored if `--subnet-config-content` is specified.

Example: Let's say we have a subnet with ID `p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6`. We can create a config file under the default `subnet-config-dir` at `$HOME/.ezcgo/configs/subnets/p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6.json`. An example config file is:
...
Subnet configs supports loading new consensus parameters. JSON keys are different from their matching `CLI` keys. These parameters must be grouped under `consensusParameters` key. The consensus parameters of a subnet default to the same values used for the Primary Network, which are given [CLI Snow Parameters](ezcgo-config-flags.md#snow-parameters).
...
| --snow-ezchain-batch-size      | batchSize             |
| --snow-ezchain-num-parents     | parentSize            |

##### Gossip Configs

It's possible to define different Gossip configurations for each subnet without changing values for Primary Network. For example in Primary Network transaction mempools are not gossipped to non-validators (`--consensus-app-gossip-non-validator-size` is `0`). You can change this for your subnet and share mempool with non-validators as well. JSON keys of these parameters are different from their matching `CLI` keys. These parameters default to the same values used for the Primary Network. For more information see [CLI Gossip Configs](ezcgo-config-flags.md#gossiping).
...
Specifies where to find EZChainGo & plugin binaries. Defaults to the path of executed EZChainGo binary. The structure of this directory must be as follows:
...
Transaction fee, in wei, for transactions that create new state. Defaults to `1000000` wei (.001 EZC) per transaction.

#### `--min-delegator-stake` (int):

The minimum stake, in wei, that can be delegated to a validator of the Primary Network.

Defaults to `25000000000` (25 EZC) on Mainnet. Defaults to `5000000` (.005 EZC) on Test Net.
...
The minimum stake, in wei, required to validate the Primary Network.

Defaults to `2000000000000` (2,000 EZC) on Mainnet. Defaults to `5000000` (.005 EZC) on Test Net.
...
The maximum stake, in wei, that can be placed on a validator on the primary network. Defaults to `3000000000000000` (3,000,000 EZC) on Mainnet. This includes stake provided by both the validator and by delegators to the validator.
...
The maximum stake supply, in wei, that can be placed on a validator. Defaults to `720,000,000,000,000,000` wei.

#### `--tx-fee` (int):

The required amount of wei to be burned for a transaction to be valid on the X-Chain, and for import/export transactions on the P-Chain. This parameter requires network agreement in its current form. Changing this value from the default should only be done on private networks. Defaults to `1,000,000` wei per transaction.
...
If profiling enabled, node continuously runs memory/CPU profiles and puts them at this directory. Defaults to the `$HOME/.ezcgo/profiles/`.
...
These flags govern rate-limiting of inbound and outbound messages. For more information on rate-limiting and the flags below, see package `throttling` in EZChainGo.
...
Path to JSON file that defines aliases for Virtual Machine IDs. Defaults to `~/.ezcgo/configs/vms/aliases.json`. This flag is ignored if `--vm-aliases-file-content` is specified. Example content:
