...
* **`AssetID`** is a 32-byte array that defines which asset this output references. The only valid `AssetID` is the EZC `AssetID`.
...
* **`AssetID`** is a 32-byte array that defines which asset this input references. The only valid `AssetID` is the EZC `AssetID`.
...
A [secp256k1](cryptographic-primitives.md#secp-256-k1-addresses) transfer output allows for sending a quantity of an asset to a collection of addresses after a specified unix time. The only valid asset is EZC.
...
* **`Validator`** Validator has a `NodeID`, `StartTime`, `EndTime`, and `Weight`
  ...
  * **`EndTime`** is a long which is the Unix time when the delegator stops delegating (and staked EZC is returned).
