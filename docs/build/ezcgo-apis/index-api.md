...
If `--index-enabled` is changed to `false` from `true`,  EZChainGo won't start as doing so would cause a previously complete index to become incomplete, unless the user explicitly says to do so with `--index-allow-incomplete`. This protects you from accidentally running with indexing disabled, after previously running with it enabled, which would result in an incomplete index.

This document shows how to query data from EZChainGo's Index API. The Index API is only ezc.lable when running with `--index-enabled`.

## Go Client

There is a Go implementation of an Index API client. See documentation [here](https://pkg.go.dev/github.com/EZChain-core/ezcgo/indexer#Client). This client can be used inside a Go program to connect to an EZChainGo node that is running with the Index API enabled and make calls to the Index API.
...
:::warning
To help users to try out this example and other index APIs, we have set up a testing indexer node located at https://indexer-demo.ezchain.com. This indexer node is not for production use. We may change or shut it down at any time without notice.
:::
...
```sh
curl --location --request POST 'https://indexer-demo.ezchain.com/ext/index/X/tx' \
--header 'Content-Type: application/json' \
--data-raw '{
   "jsonrpc": "2.0",
   "method": "index.getContainerByIndex",
...
}' -H 'content-type:application/json;' https://api.ezchain.com/ext/bc/X
```
        ...
        "outputs": [
          {
            ...
            "output": {
              "addresses": [
                "X-ezc1wst8jt3z3fm9ce0z6akj3266zmgccdp03hjlaj"
              ],
              "amount": 4999000000,
              "locktime": 0,
              "threshold": 1
          ...
          },
          {
            ...
            "output": {
              "addresses": [
                "X-ezc1slt2dhfu6a6qezcn5sgtagumq8ag8we75f84sw"
              ],
