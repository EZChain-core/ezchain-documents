...
This API allows clients to interact with the [P-Chain](../../learn/platform-overview/README.md#platform-chain-p-chain), which maintains EZChain’s [validator](../../learn/platform-overview/staking.md#validators) set and handles blockchain creation.
...
A delegator stakes EZC and specifies a validator (the delegatee) to validate on their behalf. The delegatee has an increased probability of being sampled by other validators (weight) in proportion to the stake delegated to them.
...
* `endTime` is the Unix time when the delegator stops delegating (and staked EZC is returned).
* `stakeAmount` is the amount of wei the delegator is staking.
...
curl -X POST --data '{
    ...
    "params": {
        ...
        "rewardAddress":"P-ezc1gss39m5sx6jn7wlyzeqzm086yfq2l02xkvmecy",
        "startTime":1594102400,
        "endTime":1604102400,
        "stakeAmount":100000,
        "from": ["P-ezc1gss39m5sx6jn7wlyzeqzm086yfq2l02xkvmecy"],
...
Add a validator to the Primary Network. You must stake EZC to do this. If the node is sufficiently correct and responsive while validating, you receive a reward when end of staking period is reached. The validator’s probability of being sampled by other validators during consensus is in proportion to the amount of EZC staked.
...
There is a maximum total weight imposed on validators. This means that no validator will ever have more EZC staked and delegated to it than this value. This value will initially be set to `min(5 * amount staked, 3M EZC)`. The total value on a validator is 3 million EZC.
...
* `endTime` is the Unix time when the validator stops validating the Primary Network (and staked EZC is returned).
* `stakeAmount` is the amount of wei the validator is staking.
...
```json
{
    "jsonrpc": "2.0",
    "result": {
        "address": "P-ezc12lqey27sfujqq6mc5a3jr5av56cjsu8hg2d3hx"
    },
    "id": 1
}
```
...
curl -X POST --data '{
    ...
    "params": {
        "controlKeys":[
            "P-ezc13xqjvp8r2entvw5m29jxxjhmp3hh6lz8laep9m",
            "P-ezc165mp4efnel8rkdeqe5ztggspmw4v40j7pfjlhu"
        ],
        "threshold":2,
        "from": ["P-ezc1gss39m5sx6jn7wlyzeqzm086yfq2l02xkvmecy"],
        "changeAddr": "P-ezc103y30cxeulkjfe3kwfnpt432ylmnxux8r73r8u",
...
Send EZC from an address on the P-Chain to an address on the X-Chain or C-Chain. After issuing this transaction, you must call the X-Chain's [`avm.import`](x-chain.mdx#avmimport) or C-Chain's [`ezc.import`](c-chain.md#ezcimport) with assetID `EZC` to complete the transfer.
...
* `amount` is the amount of wei to send.
* `to` is the address on the X-Chain or C-Chain to send the EZC to.
* `from` are the addresses that you want to use for this operation. If omitted, uses any of your addresses as needed.
* `changeAddr` is the address any change will be sent to. If omitted, change is sent to one of the addresses controlled by the user.
* `username` is the user sending the EZC and paying the transaction fee.
* `password` is `username`‘s password.
...
curl -X POST --data '{
    ...
    "params": {
        "to":"X-ezc1yv8cwj9kq3527feemtmh5gkvezna5xys08mxet",
        "amount":1,
        "from": ["P-ezc1gss39m5sx6jn7wlyzeqzm086yfq2l02xkvmecy"],
        "changeAddr": "P-ezc103y30cxeulkjfe3kwfnpt432ylmnxux8r73r8u",
        "username":"myUsername",
...
curl -X POST --data '{
    ...
    "params" :{
        "username" :"myUsername",
        "password": "myPassword",
        "address": "P-ezc1zwp96clwehpwm57r9ftzdm7rnuslrunj68ua3r"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/P
```
...
Get the balance of EZC controlled by a given address.
...
* `balance` is the total balance, in wei.
* `unlocked` is the unlocked balance, in wei.
* `lockedStakeable` is the locked stakeable balance, in wei.
* `lockedNotStakeable` is the locked and not stakeable balance, in wei.
...
curl -X POST --data '{
  ...
  "params" :{
      "address":"P-ezc1m8wnvtqvthsxxlrrsu3f43kf9wgch5tyfx4nmf"
  }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/P
```
...
```json
{
  "jsonrpc": "2.0",
  "result": {
    "block": "",
    "encoding": "cb58"
  },
  "id": 1
}
            ...
            "exportedOutputs": [
              {
                ...
                "output": {
                  "addresses": [
                    "P-ezc1jkjyvlwclyu42n4yuegpczpfgwrf8r9lyj0d3c"
                  ],
                  "amount": 13838124063,
                  "locktime": 0,
                  "threshold": 1
...
* `blockchains` is all of the blockchains that exists on the EZChain network.
...
Returns an upper bound on the number of EZC that exist. This is an upper bound because it does not account for burnt tokens, including transaction fees.
...
* `supply` is an upper bound on the number of EZC that exist, denominated in wei.
...
The response in this example indicates that EZC’s supply is at most 365.865 million.
...
* `validators`:
  ...
  * `startTime` is the Unix time when the validator starts validating the Subnet.
  * `endTime` is the Unix time when the validator stops validating the Subnet.
  * `stakeAmount` is the amount of wei this validator staked. Omitted if `subnetID` is not the Primary Network.
  ...
  * `delegators` is the list of delegators to this validator:
    ...
    * `stakeAmount` is the amount of wei this delegator staked. Omitted if `subnetID` is not the Primary Network.
        ...
        "validators": [
            {
                ...
                "rewardOwner": {
                    ...
                    "threshold": "1",
                    "addresses": [
                        "P-ezc18jma8ppw3nhx5r4ap8clazz0dps7rv5u00z96u"
                    ]
                },
...
Returns the maximum amount of wei staking to the named node during a particular time period.
...
Get the minimum amount of EZC required to validate the Primary Network and the minimum amount of EZC that can be delegated.
...
Get the amount of wei staked by a set of addresses. The amount returned does not include staking rewards.
...
Get the total amount of wei staked on the Primary Network.
...
Suppose we want all UTXOs that reference at least one of `P-ezc1s994jad0rtwvlfpkpyg2yau9nxt60qqfv023qx` and `P-ezc1fquvrjkj7ma5srtayfvx7kncu7um3ym73ztydr`.
...
curl -X POST --data '{
    ...
    "params" :{
        "addresses":["P-ezc1s994jad0rtwvlfpkpyg2yau9nxt60qqfv023qx", "P-ezc1fquvrjkj7ma5srtayfvx7kncu7um3ym73ztydr"],
        "limit":5,
        "encoding": "cb58"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/P
...
```json
{
    ...
    "result": {
        ...
        "endIndex": {
            "address": "P-ezc1fquvrjkj7ma5srtayfvx7kncu7um3ym73ztydr",
...
curl -X POST --data '{
    ...
    "method" :"platform.getUTXOs",
    "params" :{
        "addresses":["P-ezc1fquvrjkj7ma5srtayfvx7kncu7um3ym73ztydr"],
        "limit":5,
        "startIndex": {
            "address": "P-ezc1fquvrjkj7ma5srtayfvx7kncu7um3ym73ztydr",
            "utxo": "kbUThAUfmBXUmRgTpgD6r3nLj7rJUGho6xyht5nouNNypH45j"
...
Complete a transfer of EZC from the X-Chain, or C-Chain to the P-Chain.

Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.mdx#avmexport) or C-Chain’s [`ezc.export`](c-chain.md#ezcexport) method with assetID `EZC` to initiate the transfer.
...
* `to` is the ID of the address the EZC is imported to. This must be the same as the `to` argument in the corresponding call to the X-Chain’s or C-Chain's `export`.
...
curl -X POST --data '{
    ...
    "params": {
        "to": "P-ezc1apzq2zt0uaaatum3wdz83u4z7dv4st7l5m5n2a",
        "from": ["P-ezc1gss39m5sx6jn7wlyzeqzm086yfq2l02xkvmecy"],
        "changeAddr": "P-ezc103y30cxeulkjfe3kwfnpt432ylmnxux8r73r8u",
        "username": "myUsername",
        "password": "myPassword"
...
```json
{
    ...
    "id": 1,
    "result": {
        "address":"P-ezc19hwpvkx2p5q99w87dlpfhqpt3czyh8ywasfaym"
    }
}
```
...
```json
{
    "jsonrpc": "2.0",
    "result": {
        "addresses": ["P-ezc1ffksh2m592yjzwfp2xmdxe3z4ushln9s09z5p0"]
    },
    "id": 1
}
```
