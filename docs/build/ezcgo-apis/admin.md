...
This API can be used for measuring node health and debugging. Note that the Admin API is disabled by default for security reasons. To run a node with the Admin API enabled, use [config flag](../references/ezcgo-config-flags.md) `--api-admin-enabled=true`.
