---
description: More information ezc.lable regarding EZChain Go APIs and learning how to interact with the C-Chain. 
sidebar_position: 4
---

# Contract Chain (C-Chain) API

_Note: Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to EZChain’s view of networkID and chainID and are purely internal to the_ [_C-Chain_](../../learn/platform-overview/README.md#contract-chain-c-chain)_. On Mainnet, the C-Chain uses `1` and `2612` for these values. On the Fuji Testnet, it uses `5` and `2613` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods._

## Deploying a Smart Contract

[Deploy a Smart Contract on EZChain Using Remix and MetaMask](../tutorials/smart-contracts/deploy-a-smart-contract-on-ezchain-using-remix-and-metamask.md)
...
EZChain offers an API interface identical to Geth's API except that it only supports the following services:
...
In addition to the standard Ethereum APIs, EZChain offers `eth_getAssetBalance` to retrieve the balance of first class EZChain Native Tokens on the C-Chain (excluding EZC, which must be fetched with `eth_getBalance`).
...
## EZChain Specific APIs

### Endpoints

To interact with the `ezc` specific RPC calls on the C-Chain:

```
/ext/bc/C/ezc
```

To interact with other instances of the EVM EZC endpoints:

```
/ext/bc/blockchainID/ezc
```
...
Export an asset from the C-Chain to X-Chain or P-Chain. After calling this method, you must call the X-Chain's [`avm.import`](x-chain.mdx#avmimport) or P-Chain's [`platform.import`](p-chain.md#platformimportezc).
...
* `assetID` is the ID of the asset. To export EZC use `"EZC"` as the `assetID`.
...
curl -X POST --data '{
    ...
    "method" :"ezc.export",
    "params" :{
        "to":"X-ezc1q9c6ltuxpsqz7ul8j0h0d0ha439qt70sr3x2m0",
        "amount": 500,
...
**DEPRECATED—instead use** [**ezc.export**](c-chain.md#ezcexport).

Send EZC from the C-Chain to X-Chain or P-Chain. After calling this method, you must call the X-Chain's [`avm.import`](x-chain.mdx#avmimport) or P-Chain's [`platform.import`](p-chain.md#platformimportezc) with assetID `EZC` on the X-Chain to complete the transfer.
...
Get the private key that controls a given address. The returned private key can be added to a user with `ezc.importKey`.
...
* `privateKey` is the CB58 endcoded string representation of the private key that controls `address`. It has a `PrivateKey-` prefix and can be used to import a key via `ezc.importKey`.
...
Suppose we want all UTXOs that reference at least one of `C-ezc1yzt57wd8me6xmy3t42lz8m5lg6yruy79m6whsf`.
...
curl -X POST --data '{
    ...
    "method" :"ezc.getUTXOs",
    "params" :{
        "addresses":["C-ezc1yzt57wd8me6xmy3t42lz8m5lg6yruy79m6whsf"],
        "sourceChain": "X",
        "startIndex": {
            "address": "C-ezc1yzt57wd8me6xmy3t42lz8m5lg6yruy79m6whsf",
...
Finalize the transfer of a non-EZC or EZC from X-Chain or P-Chain to the C-Chain. Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.mdx#avmexport) or P-Chain’s [`platform.exportEZC`](p-chain.md#platformexportezc) with assetID `EZC` to initiate the transfer.
...
**DEPRECATED—instead use** [**ezc.import**](c-chain.md#ezcimport)

Finalize a transfer of EZC from the X-Chain or P-Chain to the C-Chain. Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.mdx#avmexport) or P-Chain’s [`platform.exportEZC`](p-chain.md#platformexportezc) with assetID `EZC` to initiate the transfer.
...
* `to` is the address the EZC is sent to. It should be in hex format.
* `sourceChain` is the ID or alias of the chain the EZC is being imported from. To import funds from the X-Chain, use `"X"`; for the P-Chain, use `"P"`.
* `baseFee` is the base fee that should be used when creating the transaction. If omitted, a suggested fee will be used.
* `username` is the user that controls the address that transaction will be sent from.
* `password` is `username`‘s password.
...
```sh
ezc.getAtomicTxStatus({txID: string}) -> {
  status: string,
...
This API can be used for debugging. Note that the Admin API is disabled by default. To run a node with the Admin API enabled, use [config flag](../references/ezcgo-config-flags.md#c-chain-config) `--coreth-admin-api-enabled:true`.
