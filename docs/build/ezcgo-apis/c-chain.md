...
_Note: Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to EZChain’s view of networkID and chainID and are purely internal to the_ [_C-Chain_](../../learn/platform-overview/README.md#contract-chain-c-chain)_. On Mainnet, the C-Chain uses `1` and `2612` for these values. On the Fuji Testnet, it uses `1` and `2163` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods._
