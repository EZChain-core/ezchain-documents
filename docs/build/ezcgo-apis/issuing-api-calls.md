...
This guide explains how to make calls to APIs exposed by EZChain nodes.
...
* `http-port` is the port the node listens on for HTTP calls. This is specified by [command-line argument](../references/ezcgo-config-flags.md#http-server) `http-port` (default value `9650`).
...
Unless otherwise noted, when bytes are sent in an API call/response, they are in [CB58](https://support.ezchain.com/en/articles/4587395-what-is-cb58) representation, a base-58 encoding with a checksum
