...
```ts
const xchain: AVMAPI = ezchain.XChain()
...
The first step in creating a new asset using EZChainJS is to determine the qualities of the asset. We will give the asset a name, a ticker symbol, as well as a denomination.
...
Now let's sign the transaction and issue it to the EZChain network. If successful it will return a [CB58](http://support.ezchain.com/en/articles/4587395-what-is-cb58) serialized string for the transaction ID.
