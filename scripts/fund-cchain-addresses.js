const ezchain_1 = require("ezchain");
const avm_1 = require("ezchain/dist/apis/avm");
const evm_1 = require("ezchain/dist/apis/evm");
const utils_1 = require("ezchain/dist/utils");
const sleep = (ms) => {
...
const ezchain = new ezchain_1.EZChain(ip, port, protocol, networkID);
const mstimeout = 3000;
const xchain = ezchain.XChain();
const cchain = ezchain.CChain();
const bintools = ezchain_1.BinTools.getInstance();
...
const locktime = new ezchain_1.BN(0);
const threshold = 1;
const memo = bintools.stringToBuffer(
  "AVM utility method buildExportTx to export EZC to the C-Chain from the X-Chain"
);
...
const main = async () => {
  const ezcAssetID = await xchain.getEZCAssetID();
  const getBalanceResponse = await xchain.getBalance(
    xAddressStrings[0],
    bintools.cb58Encode(ezcAssetID)
  );
  const balance = new ezchain_1.BN(getBalanceResponse.balance);
  ...
  // 1,000 EZC
  const amount = new ezchain_1.BN(1000000000000);
  console.log("Exporting 1000 EZC to each address on the C-Chain...");
  let secpTransferOutput = new avm_1.SECPTransferOutput(
    amount.mul(new ezchain_1.BN(10)),
    [cAddresses[0]],
    locktime,
    threshold
  );
  ...
  secpTransferOutput = new avm_1.SECPTransferOutput(
    balance.sub(amount.mul(new ezchain_1.BN(10))).sub(fee),
    xAddresses,
    locktime,
    threshold
  );
  ...
  console.log(avmTXID);
  await sleep(mstimeout);
  console.log("Importing EZC to the C-Chain...");
