#!/bin/bash
# Does a multi-step installation of Prometheus, Grafana, node_exporter and EZChain dashboards
...
usage () {
  ...
  echo "   --4      Step 4: Installs EZChainGo Grafana dashboards"
  echo ""
  echo "Run without any options, script will download and install latest version of EZChainGo dashboards."
}
...
install_prometheus() {
  echo "EZChainGo monitoring installer"
  echo "--------------------------------"
  echo "STEP 1: Installing Prometheus"
  echo
  get_environment
  check_reqs
  mkdir -p /tmp/ezchain-monitoring-installer/prometheus
  cd /tmp/ezchain-monitoring-installer/prometheus
  ...
  echo
  echo "You can also check Prometheus web interface, ezc.lable on http://your-node-host-ip:9090/"
  echo
  echo "If everything looks ok you can now continue with installing Grafana. Refer to the tutorial:"
  echo "https://docs.ezchain.com/build/tutorials/nodes-and-staking/setting-up-node-monitoring#grafana"
  echo
  echo "Reach out to us on https://t.me/EZChainOfficia if you're having problems."
...
install_grafana() {
  ...
  echo
  echo "You can also check Grafana web interface, ezc.lable on http://your-node-host-ip:3000/"
  echo
  echo "Now you need to set up Prometheus as a data source for Grafana. Refer to the tutorial:"
  echo "https://docs.ezchain.com/build/tutorials/nodes-and-staking/setting-up-node-monitoring#exporter"
  echo
  echo "Reach out to us on https://t.me/EZChainOfficia if you're having problems."
...
install_exporter() {
  ...
  echo
  get_environment
  mkdir -p /tmp/ezchain-monitoring-installer/exporter_archive
  cd /tmp/ezchain-monitoring-installer/exporter_archive
  ...
  tar xvf node_exporter.tar.gz -C /tmp/ezchain-monitoring-installer/exporter_archive --strip-components=1
  sudo mv /tmp/ezchain-monitoring-installer/exporter_archive/node_exporter /usr/local/bin
  ...
  echo "https://docs.ezchain.com/build/tutorials/nodes-and-staking/setting-up-node-monitoring#dashboards"
  echo
  echo "Reach out to us on https://t.me/EZChainOfficia if you're having problems."
}
...
install_dashboards() {
  ...
  if test -f "/etc/grafana/grafana.ini"; then
    ...
    echo "https://docs.ezchain.com/build/tutorials/nodes-and-staking/setting-up-node-monitoring"
    echo
    usage
    exit 0
  fi

  if test -f "/etc/grafana/provisioning/dashboards/ezchain.yaml"; then
  ...
  echo
  echo "Downloading..."
  mkdir -p /tmp/ezchain-monitoring-installer/dashboards-install
  cd /tmp/ezchain-monitoring-installer/dashboards-install

  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/c_chain.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/database.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/machine.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/main.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/network.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/p_chain.json
  wget -nd -m -nv https://raw.githubusercontent.com/EZChain-core/ezchain-docs/master/dashboards/x_chain.json
  ...
  if [ "$provisioningDone" = "false" ]; then
    ...
    sudo cp ezchain.yaml /etc/grafana/provisioning/dashboards/
  ...
  echo "Done!"
  echo
  echo "EZChainGo Grafana dashboards have been installed and updated."
...
then
  case $1 in
    ...
    --3) #install node_exporter
      ...
      exit 0
      ;;
    --4) #install EZChainGo dashboards
      install_dashboards
      exit 0
      ;;
    --help)
